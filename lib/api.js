 ...
 * POST /api/v2/orders
 *
 * @param {Object} orderData - Complete order object from CartManager.prepareOrder()
 * @returns {Promise<Object>} Order response with id, booking_url, customer_id
 */
export async function createOrder(orderData) {
  if (!orderData || !orderData.customer) {
    throw new Error('Invalid order data: customer is required');
  }
  if (!orderData.products || !Array.isArray(orderData.products) || orderData.products.length === 0) {
    throw new Error('Invalid order data: products array is required and must not be empty');
  }
